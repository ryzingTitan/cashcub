import {
  ...
  describe("Transaction API functions", () => {
    ...
    it("deleteTransaction should delete a transaction successfully", async () => {
      await expect(deleteTransaction("/transactions", "1")).resolves.toBeUndefined();
    });
    ...
    it("getAllTransactions should reject with an error if the fetch fails", async () => {
        ...
        await expect(getAllTransactions("/transactions")).rejects.toEqual("Failed to fetch transactions");
    });
    ...
    it("createTransaction should reject with an error if the fetch fails", async () => {
        ...
        await expect(createTransaction("/transactions", { amount: 200 })).rejects.toEqual("Failed to create transaction");
    });
    ...
    it("updateTransaction should reject with an error if the fetch fails", async () => {
        ...
        await expect(updateTransaction("/transactions", "1", { amount: 150 })).rejects.toEqual("Failed to update transaction");
    });
    ...
    it("deleteTransaction should reject with an error if the fetch fails", async () => {
        ...
        await expect(deleteTransaction("/transactions", "1")).rejects.toEqual("Failed to delete transaction");
    });
  });
