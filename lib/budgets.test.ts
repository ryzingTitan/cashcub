...
describe("Budget API functions", () => {
  ...
  it("updateBudgetItem should update a budget item successfully", async () => {
    const data = await updateBudgetItem("/budget-items/1", {
      name: "Updated Item",
    });
  ...
  it("getAllBudgets should reject with an error if the fetch fails", async () => {
    ...
    await expect(getAllBudgets("/budgets")).rejects.toEqual(
  ...
  it("cloneBudget should reject with an error if the fetch fails", async () => {
    ...
    await expect(cloneBudget("/budgets/1/clone", {})).rejects.toEqual(
      "Failed to clone budget",
    );
  });
  ...
  it("createBudgetItem should reject with an error if the fetch fails", async () => {
    ...
    await expect(
      createBudgetItem("/budget-items", { name: "New Item" }),
    ).rejects.toEqual("Failed to create budget item");
  });
  ...
  it("deleteBudgetItem should reject with an error if the fetch fails", async () => {
    ...
    await expect(deleteBudgetItem("/budget-items/1")).rejects.toEqual(
  ...
  it("updateBudgetItem should reject with an error if the fetch fails", async () => {
    ...
    await expect(
      updateBudgetItem("/budget-items/1", { name: "Updated Item" }),
    ).rejects.toEqual("Failed to update budget item");
  });
  ...
  it("getBudgetSummary should reject with an error if the fetch fails", async () => {
    ...
    await expect(getBudgetSummary("/budgets/summary/1")).rejects.toEqual(
